...
var Orderbooks = make(map[string]*arb.OrderbookData) //pointer seems like a bad idea but makes assignment of elements easier
var ArbContainer = arb.ArbTablesContainer{ArbTables: make(map[string]*arb.ArbTable)}
var AevoIndex = arb.IndexContainer{Index: make(map[string]float64)}
var LyraIndex = arb.IndexContainer{Index: make(map[string]float64)}
...
func unpackOrders(orders []interface{}, exchange string) ([]arb.Order, error) {
	unpackedOrders := make([]arb.Order, 0)
	for _, order := range orders {
		...
		unpackedOrders = append(unpackedOrders, arb.Order{price, amount, iv, exchange})
	}
...
func lyraUpdateOrderbooks(data map[string]interface{}) {
	...
	if exists {
		...
		Orderbooks[instrument] = &arb.OrderbookData{
			Bids:        bids,
			Asks:        asks,
...
func lyraWssRead(ctx context.Context, c *websocket.Conn) {
	...
	if strings.Contains(channel, "spot_feed") {
		...
		arb.UpdateArbTables(&ArbContainer, &Orderbooks, &AevoIndex, &LyraIndex, "ETH")
...
func arbTableHandler(w http.ResponseWriter, r *http.Request) {
	...
	arbTablesSlice := make([]*arb.ArbTable, len(ArbContainer.ArbTables))
	i := 0
